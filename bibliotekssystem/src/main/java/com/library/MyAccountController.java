...
public class MyAccountController {
    ...
    public void initialize() {
        ...
        accountListView.setOnMouseClicked(event -> {
            if (event.getClickCount() == 2) {
                ...
                if (sectionTitleText.getText().contains("Mina köplatser")) {
                    try {
                        ...
                        System.err.println("Kunde inte öppna objektsidan: " + e.getMessage());
                    }
                }
                
                else if (sectionTitleText.getText().contains("Tidigare lån")) {
                    try {
                        int start = "Kvitto: ".length();
                        int end = selected.indexOf(" (");
                        
                        if (end > start) {
                            String receiptNumber = selected.substring(start, end);
                            
                            String receiptText = userDAO.getFormattedReceipt(receiptNumber);
                            
                            if (!receiptText.isEmpty()) {
                                showReceiptPopup(receiptText);
                            }
                        }
                    } catch (Exception e) {
                        System.err.println("Kunde inte ladda popup-kvitto: " + e.getMessage());
                    }
                }
            }
        });
    ...
    private void showReceiptPopup(String receiptText) {
        javafx.scene.control.Alert alert = new javafx.scene.control.Alert(javafx.scene.control.Alert.AlertType.INFORMATION);
        alert.setTitle("Arkiverat Lånekvitto");
        alert.setHeaderText("Här är det sparade kvittot för detta lånetillfälle:");
