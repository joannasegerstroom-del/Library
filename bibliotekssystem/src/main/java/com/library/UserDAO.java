...
public class UserDAO {
    ...
    public String getFormattedReceipt(String receiptNumber) {
        StringBuilder sb = new StringBuilder();
        String sql = "SELECT cs.loandate, u.name, l.barcode, l.duedate, i.title " +
                     "FROM checksession cs " +
                     "JOIN LibraryUser u ON cs.userid = u.userID " +
                     "JOIN Loan l ON cs.receiptnumber = l.receiptnumber " +
                     "JOIN Copy c ON l.barcode = c.barcode " +
                     "JOIN Item i ON c.itemID = i.itemID " +
                     "WHERE cs.receiptnumber = ?";
        ...
        try (Connection conn = DatabaseManager.getConnection();
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                ...
                while (rs.next()) {
                    if (!headerBuilt) {
                        sb.append("=====================================\n");
                        sb.append("         BIBLIOTEKSKVITTO\n");
                        sb.append("=====================================\n");
                        sb.append("Datum: ").append(rs.getDate("loandate")).append("\n");
                        sb.append("Låntagare: ").append(rs.getString("name")).append("\n");
                        sb.append("Kvittonummer: ").append(receiptNumber).append("\n");
                        sb.append("-------------------------------------\n");
                        headerBuilt = true;
                    }
                    sb.append("Titel: ").append(rs.getString("title")).append("\n");
                    sb.append("Identifierare: ").append(rs.getString("barcode")).append("\n");
                    sb.append("Återlämnas senast: ").append(rs.getDate("duedate")).append("\n\n");
                }
                
                if (headerBuilt) {
                    sb.append("-------------------------------------\n");
                    sb.append("Historiskt kvitto från arkivet.\n");
                }
            }
        } catch (SQLException e) {
            System.err.println("Fel vid hämtning av historiskt kvitto: " + e.getMessage());
        }
        return sb.toString();
    }

    public List<String> getAllOverdueLoans() {
        List<String> overdueList = new ArrayList<>();
        String sql = "SELECT u.name, u.email, i.title, l.barcode, l.duedate " +
                     "FROM checksession cs " +
                     "JOIN LibraryUser u ON cs.userid = u.userID " +
                     "JOIN Loan l ON cs.receiptnumber = l.receiptnumber " +
                     "JOIN Copy c ON l.barcode = c.barcode " +
                     "JOIN Item i ON c.itemID = i.itemID " +
                     "WHERE c.status = 'LOANED' AND l.returndate IS NULL AND l.duedate < CURRENT_DATE " +
        ...
        try (Connection conn = DatabaseManager.getConnection();
            ...
            while (rs.next()) {
                String info = rs.getString("name") + " (" + rs.getString("email") + ") | " +
                              rs.getString("title") + " #" + rs.getString("barcode") + 
                              " | Förföll: " + rs.getDate("duedate");
                overdueList.add(info);
            }
        } catch (SQLException e) {
            System.err.println("Fel vid hämtning av alla försenade lån: " + e.getMessage());
        }
        return overdueList;
    }

    public boolean markCopyAsLost(String barcode) {
        String sql = "UPDATE Copy SET status = 'LOST' WHERE barcode = ?";
        try (Connection conn = DatabaseManager.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            System.err.println("Fel vid markering av försvunnet objekt: " + e.getMessage());
        }
        return false;
    }
